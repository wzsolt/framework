...
$(document).ready(function() {
    ...
    var $fileuploader = $('input.gallery_media').fileuploader({
        limit: 500,
        fileMaxSize: 5,
        extensions: ['jpg', 'jpeg', 'png', 'gif', 'image/*', 'video/*'],
        changeInput: ' ',
        theme: 'gallery',
        enableApi: true,
        thumbnails: {
            box: '<div class="fileuploader-items">' +
                    '<ul class="fileuploader-items-list">' +
                        '<li class="fileuploader-input"><button type="button" class="fileuploader-input-inner"><i class="fas fa-cloud-upload-alt"></i> <span>${captions.feedback}</span></button></li>' +
                    '</ul>' +
                '</div>',
            item: '<li class="fileuploader-item file-main-0">' +
                    '<div class="fileuploader-item-inner">' +
                        '<div class="actions-holder">' +
                            '<button type="button" class="fileuploader-action fileuploader-action-mainselector is-hidden float-left"><i class="fas fa-main-selector fa-2x gallery-action-asmain"></i></button>' +
                            '<button type="button" class="fileuploader-action fileuploader-action-sort is-hidden" title="${captions.sort}"><i class="fas fa-arrows-alt"></i></button>' +
                            '<button type="button" class="fileuploader-action fileuploader-action-settings is-hidden" title="${captions.edit}"><i class="fas fa-cog"></i></button>' +
                            '<button type="button" class="fileuploader-action fileuploader-action-remove" title="${captions.remove}"><i class="fas fa-trash-alt"></i></button>' +
            ...
            item2: '<li class="fileuploader-item file-main-${data.isMain}">' +
                    '<div class="fileuploader-item-inner">' +
                        '<div class="actions-holder">' +
                            '<button type="button" class="fileuploader-action fileuploader-action-mainselector float-left"><i class="fas fa-main-selector fa-2x gallery-action-asmain"></i></button>' +
                            '<button type="button" class="fileuploader-action fileuploader-action-sort" title="${captions.sort}"><i class="fas fa-arrows-alt"></i></button>' +
                            '<button type="button" class="fileuploader-action fileuploader-action-settings" title="${captions.edit}"><i class="fas fa-cog"></i></button>' +
                            '<button type="button" class="fileuploader-action fileuploader-action-remove" title="${captions.remove}"><i class="fas fa-trash-alt"></i></button>' +
                            '<div class="gallery-item-dropdown">' +
        ...
        upload: {
            ...
            onSuccess: function(result, item) {
                ...
                setTimeout(function() {
                    ...
                    item.html.find('.fileuploader-action-mainselector').removeClass('is-hidden');
                }, 400);
            },
                ...
                if ($target.is('.gallery-action-asmain') && item.data.listProps) {
                    ...
                    $.post('/ajax/uploader/gallery/' + folder + '/mark/', {name: item.name, id: item.data.listProps.id}, function(result) {
                        ...
                        if(result.selected){
                            ...
                            item.html.removeClass('file-main-1').addClass('file-main-0');
